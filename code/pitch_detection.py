...
def augmented_detect_pitch_CMNDF(f, W, t, sample_rate, bounds, thresh=0.01):  # Also uses memoization
...
def correct_augmented_detect_pitch_CMNDF(f, W, t, sample_rate, bounds, thresh=0.05):
    CMNDF_vals = memo_CMNDF(f, W, t, bounds[-1])[bounds[0]:]
    lower_bound = max(bounds[0], 0)
    upper_bound = min(bounds[1], len(CMNDF_vals))
    sample = np.argmin(
...
def f(x):
    f_0 = 1
    def envelope(x): return np.exp(-x)
...
def singer_main():
    sample_rate, data = wavfile.read(Path(__file__).with_name("happy_birthday.wav"))
    ...
    for i in tqdm(range(data.shape[0] // (window_size + 3))):
        pitches.append(
            correct_augmented_detect_pitch_CMNDF(
                data,
                window_size,
                i * window_size,
                sample_rate,
    ...
    plt.plot(pitches, marker='.', linestyle='None', color='#1f77b4', label="s = 0.15")
    # plt.ylim(300, 600)
    ...
    plt.plot(pitches, marker='.', linestyle='None', color='#ff7f0e', label="s = 0.01")
    plt.ylim(0, 550)
    plt.xlim(0, 4321)
    plt.legend()
    plt.show()
