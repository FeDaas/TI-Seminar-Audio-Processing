...
happy_birthday_notes = [0, 60, 62, 64, 65, 67, 69, 72]
...
def closest_pitch(f0):
    ...
    rounded_array = np.array([happy_birthday_notes[np.argmin(
        np.abs(happy_birthday_notes - num))] for num in midi_note])

    rounded_array = np.where(rounded_array == 0, np.nan, rounded_array)
    # Convert back to Hz.
    return librosa.midi_to_hz(rounded_array)
...
def autotune(audio, sr, correction_function, plot=False):
    ...
    if plot:
        ...
        fig, ax = plt.subplots()
        fig.set_size_inches(12.4125, 1.5625)
        img = librosa.display.specshow(
            log_stft, x_axis='time', y_axis='log', ax=ax, sr=sr, hop_length=hop_length, fmin=fmin,
            fmax=fmax)
        fig.colorbar(img, ax=ax, format="%+2.f dB")
        ax.plot(time_points, f0, label='original pitch', color='cyan', linewidth=2)
        ax.plot(time_points, corrected_f0, label='corrected pitch', color='red', linewidth=1.5)
